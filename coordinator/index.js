...
const CoordinatorTasks = ["Move to closed refund requests"];
...
function CreateTaskView() {
    ...
    taskService.list({}, function (err, results) {
        ...
        results._embedded.task.forEach(task => {
            ...
            if (!CoordinatorTasks.includes(task.name)) return;
            const row = `
            <tr>
                <td>${task.id}</td>
                <td>${task.name}</td>
    ...
    taskService.list({}, function (err, results) {
        ...
        results._embedded.task.forEach(task => {
            ...
            <tr>
                ...
                <td>${task.assignee}</td>
                <td>
                    <button class="btn btn-primary" onclick="window.location.href = '/coordinator/task/?id=${task.id}'">Fill in form</button>
                </td>
            </tr>
            `;
...
function main() {
    if (!userId) window.location.search = 'u=jure';
    CreateTaskView();
    CreateMyTaskView();
}
window.onload = main;